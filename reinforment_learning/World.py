...
from Helpers import ACTION as A, WORLD as W
...
class World:
    def __init__(self, filename, reward=default_reward, random_rate=default_random_rate, time_limit=default_time_limit):
        file = open(filename)
        self.map = np.array(
    ...
    def get_next_tile(self, pos, action):
        """
        Get the next tile given the current position and the action.
    ...
    def get_transition_model(self):
        """
        Calculate the transitional model p(s'|s, a) of MDP.
        ...
        for r in range(self.n_rows):
            for c in range(self.n_cols):
                ...
                if self.map[curr_pos] == 0:
                    for a in A.ACTIONS:
                        next_pos = self.get_next_tile(curr_pos, a)
                ...
                for a in A.ACTIONS:
                    ...
                    transition_model[s, a, int(neighbor_s[(a + 1) % A.LEN])] += self.random_rate / 2.0
                    transition_model[s, a, int(neighbor_s[(a - 1) % A.LEN])] += self.random_rate / 2.0
